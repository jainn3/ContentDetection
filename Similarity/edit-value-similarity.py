...
def computeScores(inputDir, outCSV, acceptTypes, allKeys):
    ...
    with open(outCSV, "wb") as outF:
        ...
        for file1, file2 in files_tuple:
            ...
            try:
                intersect_features = set(ast.literal_eval(file1_parsedData["content"]).keys()) & set(ast.literal_eval(file2_parsedData["content"]).keys())                
            ...
            for feature in intersect_features:
                try:
                    file1_feature_value = stringify(ast.literal_eval(file1_parsedData["content"])[feature])
                    file2_feature_value = stringify(ast.literal_eval(file2_parsedData["content"])[feature])
            ...
            if allKeys:
                file1_only_features = set(ast.literal_eval(file1_parsedData["content"]).keys()) - set(intersect_features)
                file1_only_features = [feature for feature in file1_only_features if feature not in na_metadata]

                file2_only_features = set(ast.literal_eval(file2_parsedData["content"]).keys()) - set(intersect_features)
